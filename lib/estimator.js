...
const afeccionesDS = loadJSON('afecciones.json');
...
// Normalizacion tolerante a errores: quita acentos, colapsa letras repetidas
// ("pechoo"->"pecho", "siii"->"si") y unifica variantes comunes de tipeo.
function normalizarFuzzy(texto) {
  return normalizar(texto)
    .replace(/(.)\1{2,}/g, '$1$1')   // 3+ repetidas -> 2
    .replace(/(.)\1/g, '$1')          // 2 repetidas -> 1 (agresivo pero util para matching)
    .replace(/[^a-z0-9\s]/g, ' ')     // quita signos
    .replace(/\s+/g, ' ')
    .trim();
}

// Distancia de Levenshtein (para tolerar 1-2 errores de tipeo por palabra).
function levenshtein(a, b) {
  const m = a.length, n = b.length;
  if (Math.abs(m - n) > 2) return 99; // corte rapido
...
// ¿Alguna palabra del texto se parece a la clave de sintoma?
// Coincidencia exacta de substring, o fuzzy con umbral proporcional a la
// longitud (evita falsos positivos: "raro" NO debe matchear "aire").
function coincideFuzzy(textoNorm, claveNorm) {
  if (textoNorm.includes(claveNorm)) return true;
  // Solo aplicamos fuzzy a claves de una palabra y suficientemente largas (>=6).
  // Tolerancia: hasta 2 errores, pero exigimos longitudes parecidas para no
  // confundir palabras distintas ("raro" NO debe matchear "aire": aire tiene 4).
  if (!claveNorm.includes(' ') && claveNorm.length >= 6) {
    const maxDist = 2;
    for (const palabra of textoNorm.split(' ')) {
      if (palabra.length < 5) continue; // palabras cortas no cuentan
      if (Math.abs(palabra.length - claveNorm.length) > 1) continue; // longitud parecida
      if (levenshtein(palabra, claveNorm) <= maxDist) return true;
    }
  }
  return false;
}
...
// Usa primero la base de conocimiento de afecciones (mas rica: incluye
// orientacion), luego las reglas simples, y por ultimo el default.
function triajePorReglas(sintomaTexto) {
  const t = normalizarFuzzy(sintomaTexto);

  // 1) Base de conocimiento de afecciones (match tolerante a errores)
  for (const af of afeccionesDS.afecciones) {
    for (const s of af.sintomas) {
      if (coincideFuzzy(t, normalizarFuzzy(s))) {
        return {
          especialidad: af.especialidad,
          urgencia: af.urgencia,
          afeccion: af.nombre,
          orientacion: af.orientacion,
          fuente: 'reglas',
        };
      }
    }
  ...
  // 2) Reglas simples por palabra clave (tolerante a errores)
  for (const regla of especialidadesDS.reglas) {
    for (const palabra of regla.palabras) {
      if (coincideFuzzy(t, normalizarFuzzy(palabra))) {
...
/** Lista compacta de afecciones para dar como contexto a la IA. */
function listarAfecciones() {
  return afeccionesDS.afecciones.map((a) => ({
    nombre: a.nombre, especialidad: a.especialidad, urgencia: a.urgencia,
  }));
}
...
/** ¿El texto contiene algun sintoma reconocible? (base de afecciones + reglas, tolerante a errores) */
function contieneSintoma(texto) {
  const t = normalizarFuzzy(texto);
  for (const af of afeccionesDS.afecciones) {
    for (const s of af.sintomas) {
      if (coincideFuzzy(t, normalizarFuzzy(s))) return true;
    }
  }
  for (const regla of especialidadesDS.reglas) {
    for (const palabra of regla.palabras) {
      if (coincideFuzzy(t, normalizarFuzzy(palabra))) return true;
    }
  }
  return false;
}
...
module.exports = {
  ...
  detectarPlan,
  contieneSintoma,
  _datasets: { especialidadesDS, planesDS, hospitalesDS, afeccionesDS },
};
