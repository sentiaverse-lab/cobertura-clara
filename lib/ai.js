...
function systemPrompt(lang) {
  const idiomaRegla = lang === 'en'
    ? '### CRITICAL LANGUAGE RULE: You MUST reply ONLY in ENGLISH. The "responder" field must always be in English, no matter what language the user writes in. ###'
    : '### REGLA DE IDIOMA CRITICA: Debes responder SOLO en ESPAÑOL. El campo "responder" siempre en español, sin importar en que idioma escriba el usuario. ###';
  return [
    idiomaRegla,
    ...
    lang === 'en' ? 'REMEMBER: the "responder" text MUST be in English.' : 'RECUERDA: el texto de "responder" DEBE estar en español.',
  ].join('\n');
}
...
async function agente(historial, estado = {}) {
  ...
  if (estado.lang === 'en') {
    partes.push('IMPORTANT: reply to the patient in ENGLISH (the "responder" field must be in English).');
  } else {
    partes.push('IMPORTANTE: responde al paciente en ESPAÑOL.');
  }
  const contexto = partes.join('\n');

  const messages = [
    { role: 'system', content: systemPrompt(estado.lang) },
