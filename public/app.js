...
// Resumen con formato bonito (emojis) para copiar/pegar en WhatsApp o notas.
function resumenTexto(est) {
  const rec = est.recomendado;
  const en = getLang() === 'en';
  const tel = rec.telefono ? rec.telefono : '';
  const lineas = en ? [
    '🩺 *COBERTURA CLARA — Estimate*',
    '',
    `👨‍⚕️ Specialty: *${est.especialidadNombre}*`,
    ...
    tel ? `📞 ${tel}` : '',
    '',
    `💵 Your copay: *${money(rec.copago)}*  (rate ${money(rec.tarifa)})`,
    `🛡️ Insurance covers: ${money(rec.cubreSeguro)}`,
    rec.claro ? `⭐ CLARO score: ${rec.claro.nota.toFixed(1)}/10` : '',
    est.masBarato && est.masBarato.id !== rec.id ? `💰 Cheapest: ${est.masBarato.nombre} (${money(est.masBarato.copago)})` : '',
    '',
    '— Generated by Vielsin · Informational only, not medical advice.',
  ] : [
    '🩺 *COBERTURA CLARA — Tu estimado*',
    '',
    `👨‍⚕️ Especialidad: *${est.especialidadNombre}*`,
    `📋 Plan: ${est.plan.nombre}`,
    '',
    `🏥 Hospital: *${rec.nombre}*`,
    rec.zona ? `📍 ${rec.zona}` : '',
    tel ? `📞 ${tel}` : '',
    '',
    `💵 Tu copago: *${money(rec.copago)}*  (tarifa ${money(rec.tarifa)})`,
    `🛡️ Cubre tu seguro: ${money(rec.cubreSeguro)}`,
    rec.claro ? `⭐ Índice CLARO: ${rec.claro.nota.toFixed(1)}/10` : '',
    est.masBarato && est.masBarato.id !== rec.id ? `💰 Más económico: ${est.masBarato.nombre} (${money(est.masBarato.copago)})` : '',
    '',
    '— Generado por Vielsin · Orientación informativa, no es consejo médico.',
  ];
...
function resumenClaro(est) {
  ...
  // Tarjeta con color de acento por categoria + animacion en cascada
  const item = (etiqueta, nombre, detalle, icon, acento, i, destacado, badge) => `
    <div class="fade-up flex items-start gap-3 p-3 rounded-2xl border ${destacado ? acento.bgOn + ' ' + acento.borderOn : 'bg-slate-900/40 border-white/5'}" style="animation-delay:${i * 0.08}s">
      <div class="shrink-0 w-9 h-9 rounded-xl ${acento.chip} flex items-center justify-center text-lg">${icon}</div>
      <div class="min-w-0 flex-1">
        <div class="text-xs font-semibold ${acento.text}">${etiqueta}</div>
        <div class="text-sm font-bold text-slate-100 truncate">${escapeHtml(nombre)}</div>
        <div class="text-xs text-slate-300 mt-0.5">${detalle}</div>
      </div>
      ${badge || ''}
    </div>`;

  const cPrecio = { text: 'text-emerald-300', chip: 'bg-emerald-500/15', bgOn: 'bg-emerald-500/10', borderOn: 'border-emerald-400/30' };
  const cCalidad = { text: 'text-amber-300', chip: 'bg-amber-500/15', bgOn: 'bg-amber-500/10', borderOn: 'border-amber-400/30' };
  const cBalance = { text: 'text-teal-300', chip: 'bg-teal-500/15', bgOn: 'bg-teal-400/10', borderOn: 'border-teal-400/40' };

  // Medalla con la nota CLARO para la ruta de balance
  const medalla = `<div class="shrink-0 text-center">
      <div class="w-11 h-11 rounded-full bg-slate-800/80 border-2 border-teal-400/50 flex items-center justify-center">
        <span class="text-base font-extrabold grad-text leading-none">${r.balance.nota.toFixed(1)}</span>
      </div>
      <div class="text-[10px] text-slate-400 mt-0.5">CLARO</div>
    </div>`;
  ...
  return `
    <div class="rounded-2xl p-4 bg-slate-900/50 border border-white/10">
      ...
      <div class="space-y-2">
        ${item(t('ifPrice'), r.precio.nombre, `${money(r.precio.copago)} · ${espera(r.precio.esperaDias)}`, '💰', cPrecio, 0, false)}
        ${item(t('ifQuality'), r.calidad.nombre, `${estrellas} ★ ${r.calidad.calidad.toFixed(1)} · ${money(r.calidad.copago)}`, '⭐', cCalidad, 1, false)}
        ${item(t('bestBalance'), r.balance.nombre, `${money(r.balance.copago)}`, '⚖️', cBalance, 2, true, medalla)}
      </div>
    </div>`;
}
